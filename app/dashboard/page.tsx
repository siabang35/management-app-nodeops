...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Jika user sampai ke halaman ini, berarti sudah authenticated dan bukan moderator
    ...
    const checkAuth = () => {
      ...
      if (hasAuthToken || hasSupabaseSession) {
        console.log("[Dashboard] ✓ User authenticated (middleware validated)")
        setLoading(false)
        return
      }
      
      // Jika tidak ada cookie sama sekali, redirect ke login
      console.warn("[Dashboard] ✗ No auth cookies found, redirecting to login")
